        ...
        .then(result => {
            if (result.success && result.usuario) {
                ...
                sessionStorage.setItem('usuario_logueado', JSON.stringify({
                    ...
                    rol: result.roles && result.roles.length > 0 ? result.roles[0].nombre : 'Electricidad'
                }));
...
async function inicializarMenu() {
    ...
    try {
        ...
        // Verificar que el usuario tenga rol de electricidad
        const rol = usuario.rol ? usuario.rol.toLowerCase() : '';
        if (rol !== 'electricidad') {
            window.location.href = getBaseUrl();
            return;
...
function cambiarContenido(opcion) {
    ...
    switch(opcion) {
        case 'Inicio':
            container.innerHTML = `
                <div class="welcome-card">
                    ...
                    <h2>Bienvenido al Panel de Electricidad</h2>
                    <p>Gestiona los Reportes de Electricidad</p>
                </div>
            ...
            `;
            setTimeout(() => {
                cargarEstadisticasDashboardElectricidad();
            }, 100);
            break;
        ...
        case 'Reportes de incidentes':
            ...
            setTimeout(() => {
                cargarReportesRecibidosElectricidad();
            }, 200);
            break;
        ...
        case 'Historial de reportes':
            container.innerHTML = `
                ...
                <div class="filtro-busqueda-container" style="margin-top: 2rem;">
                    <div class="filtro-header">
                        ...
                        <button class="btn-limpiar-filtros" onclick="limpiarFiltrosHistorialReportesElectricidad()">
                    ...
                    <div class="filtro-body">
                        <div class="filtro-row">
                            <div class="filtro-group">
                                ...
                                <input type="text" id="filtro_historial_reporte_usuario_electricidad" placeholder="Buscar por usuario..." onkeyup="aplicarFiltrosHistorialReportesElectricidad()">
                        </div>
                            <div class="filtro-group">
                                <label><i class="fas fa-calendar"></i> Fecha de Reporte</label>
                                <input type="date" id="filtro_historial_reporte_fecha_electricidad" onchange="aplicarFiltrosHistorialReportesElectricidad()">
                        </div>
                            <div class="filtro-group">
                                <label><i class="fas fa-building"></i> Oficina</label>
                                <input type="text" id="filtro_historial_reporte_oficina_electricidad" placeholder="Buscar por oficina..." onkeyup="aplicarFiltrosHistorialReportesElectricidad()">
                        </div>
                            <div class="filtro-group">
                                <label><i class="fas fa-calendar-check"></i> Fecha de Solución</label>
                                <input type="date" id="filtro_historial_reporte_fecha_solucion_electricidad" onchange="aplicarFiltrosHistorialReportesElectricidad()">
                        </div>
                    </div>
                    </div>
                </div>
                ...
                <div style="margin-top: 1.5rem; display: flex; justify-content: flex-end;">
                    <button class="btn-exportar-pdf" onclick="exportarHistorialReportesElectricidadPDF()" style="background: linear-gradient(135deg, #e74c3c, #c0392b); color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 8px; cursor: pointer; font-size: 1rem; font-weight: 600; display: inline-flex; align-items: center; gap: 0.5rem; transition: all 0.3s ease; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
                        <i class="fas fa-file-pdf"></i> Exportar Historial a PDF
                    </button>
                </div>
                
                <div id="historial-reportes-container-electricidad" style="margin-top: 2rem;">
                    <div id="historial-reportes-loading-electricidad" class="loading-state">
                        <i class="fas fa-spinner fa-spin"></i>
                        <p>Cargando historial...</p>
                    </div>
                    
                    <div id="historial-reportes-empty-electricidad" class="empty-state" style="display: none;">
                        <i class="fas fa-inbox"></i>
                        <p>No hay reportes resueltos</p>
                    </div>
                    
                    <div id="historial-reportes-grid-electricidad" style="display: none;">
                        <!-- Los reportes se cargarán aquí -->
                    </div>
                    
                    <!-- Controles de paginación -->
                    <div class="paginacion-container" id="historial-reportes-paginacion-container-electricidad" style="display: none;">
                        <div class="paginacion-info">
                            <span id="historial-reportes-paginacion-info-texto-electricidad">Página 1 de 1</span>
                        </div>
                        <div class="paginacion-controls">
                            <button id="historial-reportes-btn-pagina-anterior-electricidad" class="btn-pagina" onclick="cambiarPaginaHistorialReportesElectricidad(-1)" disabled>
                                <i class="fas fa-chevron-left"></i> Anterior
                            </button>
                            <div class="paginacion-numeros" id="historial-reportes-paginacion-numeros-electricidad">
                                <!-- Los números de página se generarán aquí -->
                            </div>
                            <button id="historial-reportes-btn-pagina-siguiente-electricidad" class="btn-pagina" onclick="cambiarPaginaHistorialReportesElectricidad(1)" disabled>
            ...
            setTimeout(() => {
                cargarHistorialReportesElectricidad(1);
            }, 200);
            break;
...
// ============================================
// FUNCIONES PARA REPORTES DE INCIDENTES (ELECTRICIDAD)
// ============================================

// Cargar reportes recibidos (solo pendientes)
async function cargarReportesRecibidosElectricidad() {
                ...
                card.innerHTML = `
                    ...
                    <div class="reporte-body">
                        ...
                        <div class="reporte-acciones">
                            <button class="btn-responder-reporte" onclick="abrirModalResponderReporteElectricidad(${reporte.id_incidencia})">
...
async function abrirModalResponderReporteElectricidad(idIncidencia, esActualizacion = false) {
    ...
    // Crear modal
    let existingModal = document.getElementById('modal-responder-reporte-electricidad');
    if (existingModal) {
        existingModal.remove();
    }
    
    let overlay = document.createElement('div');
    overlay.id = 'modal-responder-reporte-electricidad';
    overlay.className = 'overlay-blur';
    overlay.style.display = 'flex';
    document.body.appendChild(overlay);
    ...
    modal.innerHTML = `
        <div class="modal-header">
            ...
            <button class="close-modal" onclick="cerrarModalResponderReporteElectricidad()">
                <i class="fas fa-times"></i>
            </button>
        </div>
        <div class="modal-body">
            <form id="formResponderReporteElectricidad" onsubmit="responderReporteElectricidad(event, ${idIncidencia}, ${idUsuario}, ${esActualizacion})">
                <div class="form-group">
                    <label for="descripcion_solucion_electricidad">Descripción de la Solución *</label>
                    <textarea id="descripcion_solucion_electricidad" name="descripcion_solucion" rows="6" required placeholder="Describe detalladamente la solución aplicada...">${escapeHtml(descripcionSolucionActual)}</textarea>
                </div>
                <div class="form-group">
                    <label for="resultado_electricidad">Resultado (Opcional)</label>
                    <input type="text" id="resultado_electricidad" name="resultado" placeholder="Ej: Solucionado, En proceso, Requiere seguimiento..." value="${escapeHtml(resultadoActual)}">
                </div>
                <div class="modal-actions">
                    <button type="button" class="btn-modal-cancel" onclick="cerrarModalResponderReporteElectricidad()">
                        <i class="fas fa-times"></i> Cancelar
                    </button>
    ...
    overlay.addEventListener('click', function(e) {
        if (e.target === overlay) {
            cerrarModalResponderReporteElectricidad();
        }
    });
...
function cerrarModalResponderReporteElectricidad() {
    const modal = document.getElementById('modal-responder-reporte-electricidad');
    if (modal) {
        modal.remove();
    }
}
...
// Responder reporte
async function responderReporteElectricidad(event, idIncidencia, idUsuario, esActualizacion = false) {
    event.preventDefault();
    
    const form = document.getElementById('formResponderReporteElectricidad');
    ...
    try {
        ...
        if (result.success) {
            ...
            if (container) {
                ...
                if (contenido.includes('historial-reportes-container-electricidad')) {
                    cargarHistorialReportesElectricidad(1);
                } else {
                    cargarReportesRecibidosElectricidad();
                }
...
let paginaActualHistorialReportesElectricidad = 1;

// Cargar historial de reportes (solo resueltos)
async function cargarHistorialReportesElectricidad(pagina = 1) {
    paginaActualHistorialReportesElectricidad = pagina;
    
    const loadingState = document.getElementById('historial-reportes-loading-electricidad');
    const emptyState = document.getElementById('historial-reportes-empty-electricidad');
    const gridContainer = document.getElementById('historial-reportes-grid-electricidad');
    const paginacionContainer = document.getElementById('historial-reportes-paginacion-container-electricidad');
    ...
    try {
        ...
        if (result.success && result.data && result.data.length > 0) {
            ...
            result.data.forEach((reporte) => {
                ...
                card.innerHTML = `
                    ...
                    <div class="reporte-body">
                        ...
                        <div class="reporte-acciones">
                            <button class="btn-responder-reporte" onclick="abrirModalResponderReporteElectricidad(${reporte.id_incidencia}, true)">
            ...
            if (result.paginacion && paginacionContainer) {
                actualizarPaginacionHistorialReportesElectricidad(result.paginacion);
...
function cambiarPaginaHistorialReportesElectricidad(direccion) {
    const nuevaPagina = paginaActualHistorialReportesElectricidad + direccion;
    if (nuevaPagina >= 1) {
        cargarHistorialReportesElectricidad(nuevaPagina);
    }
}

// Ir a página específica del historial de reportes
function irAPaginaHistorialReportesElectricidad(pagina) {
    if (pagina >= 1) {
        cargarHistorialReportesElectricidad(pagina);
    }
}

// Actualizar controles de paginación del historial de reportes
function actualizarPaginacionHistorialReportesElectricidad(paginacion) {
    const infoTexto = document.getElementById('historial-reportes-paginacion-info-texto-electricidad');
    const btnAnterior = document.getElementById('historial-reportes-btn-pagina-anterior-electricidad');
    const btnSiguiente = document.getElementById('historial-reportes-btn-pagina-siguiente-electricidad');
    const numerosContainer = document.getElementById('historial-reportes-paginacion-numeros-electricidad');
    ...
    if (inicio > 1) {
        ...
        btn.textContent = '1';
        btn.onclick = () => irAPaginaHistorialReportesElectricidad(1);
    ...
    for (let i = inicio; i <= fin; i++) {
        ...
        btn.textContent = i;
        btn.onclick = () => irAPaginaHistorialReportesElectricidad(i);
        numerosContainer.appendChild(btn);
    }
    ...
    if (fin < totalPaginas) {
        ...
        btn.onclick = () => irAPaginaHistorialReportesElectricidad(totalPaginas);
        numerosContainer.appendChild(btn);
    }
}
...
function aplicarFiltrosHistorialReportesElectricidad() {
    cargarHistorialReportesElectricidad(1);
}
...
async function cargarEstadisticasDashboardElectricidad() {
    try {
        // Obtener el rol del usuario desde sessionStorage
        const usuarioLogueado = sessionStorage.getItem('usuario_logueado');
        let tipoIncidencia = 'Electricidad'; // Por defecto para electricidad
        let idUsuario = null;
        
        if (usuarioLogueado) {
            try {
                const usuario = JSON.parse(usuarioLogueado);
                const rol = usuario.rol || 'Electricidad';
        ...
        if (result.success && result.data) {
            ...
            // Total de reportes (todos los reportes de tipo Electricidad)
            document.getElementById('stat-reportes-total').textContent = stats.reportes_recibidos || 0;
            
            // Total de recibidos (igual al total, todos los reportes recibidos de tipo Electricidad)
...
function limpiarFiltrosHistorialReportesElectricidad() {
    const filtroUsuario = document.getElementById('filtro_historial_reporte_usuario_electricidad');
    const filtroOficina = document.getElementById('filtro_historial_reporte_oficina_electricidad');
    const filtroFecha = document.getElementById('filtro_historial_reporte_fecha_electricidad');
    const filtroFechaSolucion = document.getElementById('filtro_historial_reporte_fecha_solucion_electricidad');
