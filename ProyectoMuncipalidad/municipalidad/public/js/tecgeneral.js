        ...
        .then(result => {
            if (result.success && result.usuario) {
                ...
                sessionStorage.setItem('usuario_logueado', JSON.stringify({
                    ...
                    rol: result.roles && result.roles.length > 0 ? result.roles[0].nombre : 'Técnico General'
                }));
                sessionStorage.setItem('token_sesion', token);
                
                // Verificar rol
                const rol = result.roles && result.roles.length > 0 ? result.roles[0].nombre.toLowerCase() : '';
                if (rol !== 'técnico general' && rol !== 'tecnicogeneral' && rol !== 'tecnico general') {
                    window.location.replace(getBaseUrl());
                    return;
                }
...
async function inicializarMenu() {
    ...
    try {
        ...
        // Verificar que el usuario tenga rol de técnico general
        const rol = usuario.rol ? usuario.rol.toLowerCase() : '';
        if (rol !== 'técnico general' && rol !== 'tecnicogeneral' && rol !== 'tecnico general') {
            window.location.href = getBaseUrl();
            return;
...
async function cargarEstadisticasDashboardTecGeneral() {
    try {
        // Obtener el rol del usuario desde sessionStorage
        const usuarioLogueado = sessionStorage.getItem('usuario_logueado');
        let tipoIncidencia = 'Técnico General'; // Por defecto para técnico general
        let idUsuario = null;
        
        if (usuarioLogueado) {
            try {
                const usuario = JSON.parse(usuarioLogueado);
                const rol = usuario.rol || 'Técnico General';
        ...
        if (result.success && result.data) {
            ...
            // Total de reportes (todos los reportes de tipo Técnico General)
            document.getElementById('stat-reportes-total').textContent = stats.reportes_recibidos || 0;
            
            // Total de recibidos (igual al total, todos los reportes recibidos de tipo Técnico General)
        ...
        console.error('Error al cargar estadísticas del dashboard de Técnico General:', error);
...
// Función para mostrar card de confirmación
...
function cambiarContenido(opcion) {
    ...
    switch(opcion) {
        case 'Inicio':
            container.innerHTML = `
                <div class="welcome-card">
                    ...
                    <h2>Bienvenido al Panel de Técnico General</h2>
                    <p>Gestiona los Reportes de Técnico General</p>
                </div>
            ...
            `;
            setTimeout(() => {
                cargarEstadisticasDashboardTecGeneral();
            }, 100);
            break;
        ...
        case 'Reportes de incidentes':
            ...
            setTimeout(() => {
                cargarReportesRecibidosTecGeneral();
            }, 200);
            break;
        ...
        case 'Historial de reportes':
            container.innerHTML = `
                ...
                <div class="filtro-busqueda-container" style="margin-top: 2rem;">
                    <div class="filtro-header">
                        ...
                        <button class="btn-limpiar-filtros" onclick="limpiarFiltrosHistorialReportesTecGeneral()">
                    ...
                    <div class="filtro-body">
                        <div class="filtro-row">
                            <div class="filtro-group">
                                ...
                                <input type="text" id="filtro_historial_reporte_usuario_tecgeneral" placeholder="Buscar por usuario..." onkeyup="aplicarFiltrosHistorialReportesTecGeneral()">
                        </div>
                            <div class="filtro-group">
                                <label><i class="fas fa-calendar"></i> Fecha de Reporte</label>
                                <input type="date" id="filtro_historial_reporte_fecha_tecgeneral" onchange="aplicarFiltrosHistorialReportesTecGeneral()">
                        </div>
                            <div class="filtro-group">
                                <label><i class="fas fa-building"></i> Oficina</label>
                                <input type="text" id="filtro_historial_reporte_oficina_tecgeneral" placeholder="Buscar por oficina..." onkeyup="aplicarFiltrosHistorialReportesTecGeneral()">
                        </div>
                            <div class="filtro-group">
                                <label><i class="fas fa-calendar-check"></i> Fecha de Solución</label>
                                <input type="date" id="filtro_historial_reporte_fecha_solucion_tecgeneral" onchange="aplicarFiltrosHistorialReportesTecGeneral()">
                        </div>
                    </div>
                    </div>
                </div>
                ...
                <div style="margin-top: 1.5rem; display: flex; justify-content: flex-end;">
                    <button class="btn-exportar-pdf" onclick="exportarHistorialReportesTecGeneralPDF()" style="background: linear-gradient(135deg, #e74c3c, #c0392b); color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 8px; cursor: pointer; font-size: 1rem; font-weight: 600; display: inline-flex; align-items: center; gap: 0.5rem; transition: all 0.3s ease; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
                        <i class="fas fa-file-pdf"></i> Exportar Historial a PDF
                    </button>
                </div>
                
                <div id="historial-reportes-container-tecgeneral" style="margin-top: 2rem;">
                    <div id="historial-reportes-loading-tecgeneral" class="loading-state">
                        <i class="fas fa-spinner fa-spin"></i>
                        <p>Cargando historial...</p>
                    </div>
                    
                    <div id="historial-reportes-empty-tecgeneral" class="empty-state" style="display: none;">
                        <i class="fas fa-inbox"></i>
                        <p>No hay reportes resueltos</p>
                    </div>
                    
                    <div id="historial-reportes-grid-tecgeneral" style="display: none;">
                        <!-- Los reportes se cargarán aquí -->
                    </div>
                    
                    <!-- Controles de paginación -->
                    <div class="paginacion-container" id="historial-reportes-paginacion-container-tecgeneral" style="display: none;">
                        <div class="paginacion-info">
                            <span id="historial-reportes-paginacion-info-texto-tecgeneral">Página 1 de 1</span>
                        </div>
                        <div class="paginacion-controls">
                            <button id="historial-reportes-btn-pagina-anterior-tecgeneral" class="btn-pagina" onclick="cambiarPaginaHistorialReportesTecGeneral(-1)" disabled>
                                <i class="fas fa-chevron-left"></i> Anterior
                            </button>
                            <div class="paginacion-numeros" id="historial-reportes-paginacion-numeros-tecgeneral">
                                <!-- Los números de página se generarán aquí -->
                            </div>
                            <button id="historial-reportes-btn-pagina-siguiente-tecgeneral" class="btn-pagina" onclick="cambiarPaginaHistorialReportesTecGeneral(1)" disabled>
            ...
            setTimeout(() => {
                cargarHistorialReportesTecGeneral(1);
            }, 200);
            break;
...
// ============================================
// FUNCIONES PARA REPORTES DE INCIDENTES (TÉCNICO GENERAL)
// ============================================

// Cargar reportes recibidos (solo pendientes)
async function cargarReportesRecibidosTecGeneral() {
    ...
    try {
        const response = await fetch(getAppBasePath() + '/config/Reportes.php', {
            ...
            body: JSON.stringify({
                ...
                tipo_incidencia: 'Técnico General',
        ...
        if (result.success && result.data && result.data.length > 0) {
            ...
            result.data.forEach((reporte) => {
                ...
                const estadoBadge = '<span class="estado-badge estado-pendiente"><i class="fas fa-clock"></i> Pendiente</span>';
                ...
                card.innerHTML = `
                    ...
                    <div class="reporte-body">
                        ...
                        <div class="reporte-acciones">
                            <button class="btn-responder-reporte" onclick="abrirModalResponderReporteTecGeneral(${reporte.id_incidencia})">
...
async function abrirModalResponderReporteTecGeneral(idIncidencia, esActualizacion = false) {
    ...
    // Crear modal
    let existingModal = document.getElementById('modal-responder-reporte-tecgeneral');
    if (existingModal) {
        existingModal.remove();
    }
    
    let overlay = document.createElement('div');
    overlay.id = 'modal-responder-reporte-tecgeneral';
    overlay.className = 'overlay-blur';
    overlay.style.display = 'flex';
    document.body.appendChild(overlay);
    ...
    modal.innerHTML = `
        <div class="modal-header">
            ...
            <button class="close-modal" onclick="cerrarModalResponderReporteTecGeneral()">
                <i class="fas fa-times"></i>
            </button>
        </div>
        <div class="modal-body">
            <form id="formResponderReporteTecGeneral" onsubmit="responderReporteTecGeneral(event, ${idIncidencia}, ${idUsuario}, ${esActualizacion})">
                <div class="form-group">
                    <label for="descripcion_solucion_tecgeneral">Descripción de la Solución *</label>
                    <textarea id="descripcion_solucion_tecgeneral" name="descripcion_solucion" rows="6" required placeholder="Describe detalladamente la solución aplicada...">${escapeHtml(descripcionSolucionActual)}</textarea>
                </div>
                <div class="form-group">
                    <label for="resultado_tecgeneral">Resultado (Opcional)</label>
                    <input type="text" id="resultado_tecgeneral" name="resultado" placeholder="Ej: Solucionado, En proceso, Requiere seguimiento..." value="${escapeHtml(resultadoActual)}">
                </div>
                <div class="modal-actions">
                    <button type="button" class="btn-modal-cancel" onclick="cerrarModalResponderReporteTecGeneral()">
                        <i class="fas fa-times"></i> Cancelar
                    </button>
    ...
    overlay.addEventListener('click', function(e) {
        if (e.target === overlay) {
            cerrarModalResponderReporteTecGeneral();
        }
    });
...
function cerrarModalResponderReporteTecGeneral() {
    const modal = document.getElementById('modal-responder-reporte-tecgeneral');
    if (modal) {
        modal.remove();
    }
}
...
// Responder reporte
async function responderReporteTecGeneral(event, idIncidencia, idUsuario, esActualizacion = false) {
    event.preventDefault();
    
    const form = document.getElementById('formResponderReporteTecGeneral');
    ...
    try {
        ...
        if (result.success) {
            ...
            if (container) {
                ...
                if (contenido.includes('historial-reportes-container-tecgeneral')) {
                    cargarHistorialReportesTecGeneral();
                } else {
                    cargarReportesRecibidosTecGeneral();
                }
            }
...
let paginaActualHistorialReportesTecGeneral = 1;

// Cargar historial de reportes (solo resueltos)
async function cargarHistorialReportesTecGeneral(pagina = 1) {
    paginaActualHistorialReportesTecGeneral = pagina;
    
    const loadingState = document.getElementById('historial-reportes-loading-tecgeneral');
    const emptyState = document.getElementById('historial-reportes-empty-tecgeneral');
    const gridContainer = document.getElementById('historial-reportes-grid-tecgeneral');
    const paginacionContainer = document.getElementById('historial-reportes-paginacion-container-tecgeneral');
    
    if (!loadingState || !emptyState || !gridContainer) return;
    
    // Obtener valores de los filtros
    const filtros = {
        usuario: document.getElementById('filtro_historial_reporte_usuario_tecgeneral')?.value || '',
        oficina: document.getElementById('filtro_historial_reporte_oficina_tecgeneral')?.value || '',
        fecha_reporte: document.getElementById('filtro_historial_reporte_fecha_tecgeneral')?.value || '',
        fecha_solucion: document.getElementById('filtro_historial_reporte_fecha_solucion_tecgeneral')?.value || ''
    };
    ...
    try {
        ...
        if (result.success && result.data && result.data.length > 0) {
            ...
            result.data.forEach((reporte) => {
                ...
                card.innerHTML = `
                    ...
                    <div class="reporte-body">
                        ...
                        <div class="reporte-acciones">
                            <button class="btn-responder-reporte" onclick="abrirModalResponderReporteTecGeneral(${reporte.id_incidencia}, true)">
            ...
            if (result.paginacion && paginacionContainer) {
                actualizarPaginacionHistorialReportesTecGeneral(result.paginacion);
...
function cambiarPaginaHistorialReportesTecGeneral(direccion) {
    const nuevaPagina = paginaActualHistorialReportesTecGeneral + direccion;
    if (nuevaPagina >= 1) {
        cargarHistorialReportesTecGeneral(nuevaPagina);
    }
}

// Ir a página específica del historial de reportes
function irAPaginaHistorialReportesTecGeneral(pagina) {
    if (pagina >= 1) {
        cargarHistorialReportesTecGeneral(pagina);
    }
}

// Actualizar controles de paginación del historial de reportes
function actualizarPaginacionHistorialReportesTecGeneral(paginacion) {
    const infoTexto = document.getElementById('historial-reportes-paginacion-info-texto-tecgeneral');
    const btnAnterior = document.getElementById('historial-reportes-btn-pagina-anterior-tecgeneral');
    const btnSiguiente = document.getElementById('historial-reportes-btn-pagina-siguiente-tecgeneral');
    const numerosContainer = document.getElementById('historial-reportes-paginacion-numeros-tecgeneral');
    ...
    if (inicio > 1) {
        ...
        btn.textContent = '1';
        btn.onclick = () => irAPaginaHistorialReportesTecGeneral(1);
    ...
    for (let i = inicio; i <= fin; i++) {
        ...
        btn.textContent = i;
        btn.onclick = () => irAPaginaHistorialReportesTecGeneral(i);
        numerosContainer.appendChild(btn);
    }
    ...
    if (fin < totalPaginas) {
        ...
        btn.onclick = () => irAPaginaHistorialReportesTecGeneral(totalPaginas);
        numerosContainer.appendChild(btn);
    }
}

// Aplicar filtros de historial de reportes
function aplicarFiltrosHistorialReportesTecGeneral() {
    cargarHistorialReportesTecGeneral(1);
}

// Exportar historial de reportes a PDF
async function exportarHistorialReportesTecGeneralPDF() {
    try {
...
function limpiarFiltrosHistorialReportesTecGeneral() {
    const filtroUsuario = document.getElementById('filtro_historial_reporte_usuario_tecgeneral');
    const filtroOficina = document.getElementById('filtro_historial_reporte_oficina_tecgeneral');
    const filtroFecha = document.getElementById('filtro_historial_reporte_fecha_tecgeneral');
    const filtroFechaSolucion = document.getElementById('filtro_historial_reporte_fecha_solucion_tecgeneral');
